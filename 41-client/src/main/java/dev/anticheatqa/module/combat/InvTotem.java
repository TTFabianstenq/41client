...
public class InvTotem extends Module {
   public final Module.NumberSetting delay = new Module.NumberSetting("Delay", 2.0, 0.0, 10.0, 1.0);
   private int cd;

   public InvTotem() {
      super("Inv Totem", "While inventory is open, moves totem to offhand.", Category.COMBAT);
   ...
   public void onTick(class_310 c) {
      if (c.field_1724 != null && c.field_1761 != null) {
         if (c.field_1755 instanceof class_490) {
            if (this.cd > 0) {
               this.cd--;
            } else if (!c.field_1724.method_6079().method_31574(class_1802.field_8288)) {
               for (int i = 9; i < 45; i++) {
